...
> Last updated: 2026-08-10 UP-5 done — arena deploy hardening (branch feat/up5-deploy-hardening)
...
- **UP-5 done** (last arena go-live blocker from the PSC arena wing, no local task ID) — branch `feat/up5-deploy-hardening`. A live bring-up on the Mac Mini found the stack *starts* (app + redis healthy, UP-2 key enforcement live) but the submit→match loop never completes. Five defects, all fixed here.
  - **P1 — the worker could not reach the sandbox.** `docker_sandbox._run_in_docker` shells out to `docker run npcwars-sandbox:latest`, but the worker container had no `docker` CLI and no `/var/run/docker.sock`, so every submission failed closed and only the prod-forbidden `NPCWARS_ALLOW_UNSANDBOXED=1` could run a match. **Fix (docker-out-of-docker, owner's architecture call):** `Dockerfile` is now multi-stage `base → worker → app` — the `worker` stage extracts *only* the docker **client** binary (39 MB) from Docker's static bundle (no daemon/containerd, no apt repo; the host is disk-constrained) and `app` stays last so a bare `docker build .` still yields the API image. compose gives `worker` `target: worker` + the socket bind-mount; `app` gets `target: app` and deliberately **no** socket and no CLI. Untrusted code still runs only in the spawned `--network=none --read-only --memory=256m --pids-limit=50` container. **Residual risk documented plainly:** socket access is host-root-equivalent *for the worker container*. New compose service `sandbox` (profile `tools`, `image: npcwars-sandbox:latest`) builds the sandbox image on the **host** daemon — the same daemon the worker reaches through the socket. New `docker_sandbox.sandbox_preflight()` (CLI+daemon+image, never raises) is logged by the worker at startup, turning "one opaque error per job" into one unmistakable line.
  - **P2 — the worker was silent and its liveness unverifiable.** It never called `logging.basicConfig`, so `docker compose logs worker` was empty even with `PYTHONUNBUFFERED=1`; its healthcheck `python -c "print('ok')"` reported `Up (healthy)` while the poll loop was dead. **Fix:** new `server/logging_setup.py` (`configure_logging`, stdout, `force=True`, level from `NPCWARS_LOG_LEVEL`, bad value → INFO, never raises) called by **both** the worker and the app (uvicorn only configures its own loggers, so the API was equally silent). New `server/heartbeat.py`: the loop touches a heartbeat file **every cycle including idle ones**, and the compose healthcheck is now `python -m server.heartbeat`, which exits non-zero once it is missing/stale. `poll_once()` log-and-continues on both dequeue and job errors; `main()` wraps startup **and** loop so a strict-mode Redis failure explains itself before exit.
  - **P3 — silent app/worker queue split.** `_get_backend()` fell back to an in-process `InMemoryQueue` on any Redis failure — in compose that strands every job with no error. **Fix:** `NPCWARS_QUEUE_STRICT=1` (exact-match, same shape as the sandbox/keyless gates; set for both services in compose) makes an unreachable Redis raise `QueueBackendUnavailableError` instead, and does **not** leave a fallback backend behind. The dev fallback is untouched by default. Both processes log the chosen backend at startup (`init_backend()`). Backend types moved to `server/queue_backends.py` to stay inside the per-file function budget; `server.queue` re-exports them so every existing import still works.
  - **P4 — host port collision.** compose hard-mapped `8000:8000` and :8000 is taken on the target host. Now `"${ARENA_HOST_PORT:-8000}:8000"` — **default unchanged** for other users, documented in `.env.example` + deploy doc (Mac Mini uses 8010).
  - **P5 (found during this task, not in the brief; fatal to the same loop)** — `server/app.py` **hardcoded** `app.state.results_dir = "results"` and ignored `RESULTS_DIR`, and compose set no `DB_PATH` on the worker. So the worker wrote `match_NNN.json` into its own container's `/app/results` and its own `/app/data/npcwars.db` while the API read *its* own — a completed match would have been invisible at `/api/match/{id}`, `/stream` and the leaderboard even after P1–P3 were fixed. Fixed in both app and compose; `app.state.db_path` is now set too (`/health`'s sqlite probe was reading a default).
  - **Also:** `restart: unless-stopped` on app/redis/worker (not on the build-only `sandbox`).
  - **E2E:** `scripts/verify_arena_e2e.sh` (executable, self-contained, PASS/FAIL, bounded waits, dumps worker logs on failure). Asserts all five DoD items; payload built with `json.dumps` (never `printf '%b'`). Sandbox execution is proven by a three-part chain, since "a match ran" alone would not distinguish the paths: `NPCWARS_ALLOW_UNSANDBOXED` is unset *inside the worker* (so `run_sandboxed` cannot take the in-process branch), the worker can `docker image inspect` the sandbox image *through the mounted socket*, and the worker logged the submission completing with no `FAILED closed`.
  - **Verification (rig has no docker daemon, so the script itself cannot run here):** rehearsed the script end-to-end against a **real** app + **real** worker poll loop with only `docker compose` stubbed → `PASS`; then proved it is not vacuous by forcing six failures (image missing, wrong service key, no service key, worker log without the completion line, `NPCWARS_ALLOW_UNSANDBOXED=1`, `NPCWARS_QUEUE_STRICT=0`, app unreachable) — each exits 1 with a specific message. The Dockerfile's docker-CLI `RUN` pipeline was executed for real (both arch URLs HTTP 200; extracts a working `docker --version` client). All five embedded python helpers were run against real match/leaderboard data.
  - **Tests:** `test_up5_worker_runtime.py` (logging/heartbeat/poll-cycle), `test_up5_queue_strict.py`, `test_up5_sandbox_preflight.py`, `test_up5_deploy_config.py` (compose/Dockerfile/env/script shape), `test_up5_app_runtime.py`, `test_up5_arena_e2e_contract.py` (in-process rehearsal of all five E2E steps, so drift fails in CI not on the host). Ruff + `bpsai-pair arch check` clean on every touched file — `server/app.py` was **already** over the import limit on main (23 > 20) and my change would have made it 27, so the 13 route imports moved behind `server.routes.ALL_ROUTERS` (now clean). That required updating three `test_s17_integration.py` tests that grepped `app.py` for literal import lines; they now assert the property those lines were a proxy for (route present in the OpenAPI schema, with a positive control) — the same shape `test_s18_integration.py` already uses.

- **UP-4 done** (final arena go-live blocker from the PSC arena wing, no local task ID) — branch `feat/up4-sandbox-entrypoint`, **PR #69**. **The safe Docker sandbox path had never run a real match.** `Dockerfile.sandbox` copied a since-renamed `npcwars/` dir (build failure), and its `CMD` was a stub that ignored stdin and printed `{"status":"sandbox_ok"}` — but `server/docker_sandbox.py::_run_in_docker` pipes `{"bots":[...], "config":{...}}` to stdin and `json.loads` the stdout expecting a real match. So the only working executor was the opt-in, unsafe `_run_in_process`. **Fix:** new container entrypoint `sandbox_entry.py` (repo root) reads the stdin payload, compiles each bot via `load_bot_from_source`, runs `engine.game.run_match`, prints the JSON match to stdout; on any error exits non-zero with the message on stderr so `_run_in_docker`'s returncode check raises. **Import hygiene:** moved `load_bot_from_source` (+ restricted-builtins/blocked-import helpers) to `engine/bot_loader.py` (dep-light: stdlib + `engine.bot_scanner`) so the slim image imports it without the `server/` package (which drags in fastapi/redis); `server/docker_sandbox.py` re-exports it (added to `__all__`) — every existing importer unaffected. **Dockerfile.sandbox:** `FROM python:3.13-slim`, **no pip install** (engine is stdlib-only, `dependencies=[]`, verified by running a full match with fastapi/redis/discord/server all import-blocked), COPY set = `engine/ data/ sandbox_entry.py` (dropped stale `npcwars/`; `data/` needed because `engine.game` imports `data.player_profiles`; `bots/` NOT copied — `run_match` takes compiled configs). `--read-only` safe: `run_match(profiles_path=None)` and `_finalize_match` guards the only write. **`Dockerfile.sandbox.dockerignore`** (BuildKit uses it instead of `.dockerignore` for `-f Dockerfile.sandbox`) un-ignores `data/` for the no-pip sandbox build without touching the server image's `.dockerignore` (zero blast radius). **On-host proof:** `scripts/verify_sandbox_container.sh` builds the image, pipes a real 3-bot payload through the container, and asserts winner+rounds and NOT `sandbox_ok` (rig has no Docker daemon — run on a Docker host). `docs/deploy-mac-mini.md` updated: build the sandbox image at bring-up; no image → fail-closed `SandboxUnavailableError`. **Tests:** `tests/test_sandbox_entry.py` (8, no Docker needed — real `load_bot_from_source`+`run_match`). Blast-radius gate green: 205 passed (sandbox_entry, docker_sandbox, s49_sandbox, s49_integration, s56_docker_env, sandbox_fail_closed, sandbox, bot_scanner) in 6s. Ruff + `bpsai-pair arch check` clean on all touched files. **Full suite NOT run to completion locally** (wall-clock-bound on docker/youtube/subprocess integration tests, per standing convention) — ran to 74% all-green before terminating; CI is the full-suite gate. Known pre-existing `tests/test_s28_integration.py::TestMissRate::test_miss_rate` hash-seed flake untouched.

- **UP-2 done** (cross-repo go-live blocker from the PSC arena wing, no local task ID) — branch `feat/up2-key-enforcement`. **Submission key enforcement:** `get_current_player` used to auto-create a player when `X-API-Key` was *absent* (only unknown keys were 401'd), so a public tunnel would have accepted keyless bot submissions from anyone. Keyless is now 401; auto-create survives only behind exact-match `NPCWARS_ALLOW_KEYLESS == "1"` (unset/blank/`true`/`yes` all reject) — same shape as UP-1's `NPCWARS_ALLOW_UNSANDBOXED`. Legacy suite depends on keyless auto-create, so `tests/conftest.py` sets the opt-in session-wide and the enforcement tests strip it per-test. **Delegated identity:** new `NPCWARS_SERVICE_API_KEY` (single provisioned relay key, `hmac.compare_digest`); service key + `X-Player-Ref` (`^[A-Za-z0-9_-]{8,128}$`) → resolve-or-create a stable player, service key without a ref → 400, any non-service key with a ref → 403 (never silently ignored, incl. on `require_api_key` routes), env unset → ref always 403. Refs are opaque HMAC tokens from PSC: stored **only** as SHA-256 in the new `player_refs` table (`server/player_refs.py`, initialised from `init_db`), never logged, never rendered, never used as a display name. Also: `_get_client_key` now mixes a hash of the ref into the rate-limit key (one shared service key would otherwise have throttled every delegated player to 1 submission / 30s), and `server/static/editor.html` surfaces 401/403 instead of dead-ending on a disabled button. Tests: `tests/test_up2_key_enforcement.py` (45). Ruff clean; arch check clean on all touched files. **Did NOT touch `server/docker_sandbox.py`** — that is UP-1's file, in flight on `feat/up1-sandbox-fail-closed`.
  - **Bundled unrelated fix (pre-existing CI red):** `tests/test_s18_integration.py::TestRouterRegistration` walked `app.routes` keeping anything with a `.path`; FastAPI >=0.13x wraps `include_router` results in a lazy `_IncludedRouter` with no `.path`, so the probe silently stopped seeing every sub-router path. Reproduced in isolation on a clean `origin/main` worktree with fastapi 0.141.1 (what CI resolves from `fastapi>=0.115`) — which is also why main's own CI run at d727022 is red. Probe now reads the OpenAPI schema + has a positive control; verified failing→passing on both 0.128.0 and 0.141.1. **Local env is pinned at fastapi 0.128.0, so local runs cannot catch this class of break — CI resolves latest on every run.**
  - **Follow-up:** delegation is only wired into `get_current_player` (submit-bot + lobby). `require_api_key` routes (bots, cosmetics, rival, tournament) reject refs loudly rather than honouring them — if the PSC relay needs those endpoints, that is a separate task.
...
1. **Owner action on the Docker host (UP-5 cannot be closed from the dev rig):**
   `docker compose --profile tools build sandbox` → `docker compose up -d --build`
   → `NPCWARS_SERVICE_API_KEY=<key> scripts/verify_arena_e2e.sh`. That script's
   `PASS` is the actual arena go-live gate; nothing here can produce it, since
   this rig has no Docker daemon.
2. Ready to start harness wave: T73.1, T73.2, T73.3, T73.10 (fully parallel)
